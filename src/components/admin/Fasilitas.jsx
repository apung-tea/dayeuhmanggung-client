...
const Fasilitas = ({ fasilitasData, loading, onAdd, onEdit, onDelete }) => {
  ...
  const pageSize = 10;
  const totalPages = Math.ceil((fasilitasData?.length || 0) / pageSize);
  const pagedData = (fasilitasData || []).slice((page - 1) * pageSize, page * pageSize);
              ...
              {pagedData && pagedData.length > 0 ? (
                pagedData.map((item, index) => (
                  <tr key={item.id} className={`hover:bg-gray-50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900 border-r border-gray-200 border-b border-gray-200">{item.nama}</td>
