...
const Admin = () => {
  ...
  const [formArtikel, setFormArtikel] = useState({ judul: '', konten: '', gambar: '' });
  ...
  const openAddBestOf = () => {
    setEditBestOf(null);
    setFormBestOf({ judul: '', lokasi: '', deskripsi: '', durasi: '', jam_buka: '', harga_tiket: '', gambar: '' });
    setFileGambar(null);
    setPreviewGambar('');
  ...
  const handleSaveBestOf = async (e) => {
    ...
    try {
      ...
      if (fileGambar) {
        ...
        formData.append('durasi', formBestOf.durasi);
        formData.append('jam_buka', formBestOf.jam_buka);
        formData.append('harga_tiket', formBestOf.harga_tiket);
      ...
      if (isEdit) {
        if (formData) {
          ...
          await bestOfService.update(editBestOf.id, {
            ...
            durasi: formBestOf.durasi,
            jam_buka: formBestOf.jam_buka,
            harga_tiket: formBestOf.harga_tiket,
  ...
  const handleSimpanTentang = async (e) => {
    ...
    try {
      if (tentangFile) {
        // Upload gambar (dan deskripsi jika ada)
        let formData = new FormData();
        formData.append('deskripsi', tentangEdit.deskripsi || '');
        formData.append('file_foto', tentangFile);
        await tentangService.upload(formData);
      } else {
        // Update deskripsi saja
        await tentangService.update({ deskripsi_perkebunan: tentangEdit.deskripsi, visi_misi: tentangEdit.visi_misi || '' });
      }
  ...
  const openAddArtikel = () => {
    setEditArtikel(null);
    setFormArtikel({ judul: '', konten: '', gambar: '' });
    setFileGambar(null);
    setPreviewGambar('');
  ...
  const openEditArtikel = (item) => {
    setEditArtikel(item);
    setFormArtikel({ judul: item.judul, konten: item.konten || item.isi, gambar: item.gambar });
    setFileGambar(null);
